...
#include <bridging/bridge_netlink.h>
	...
	while (link_iter != 0) {
		if (rtnl_link_get_master(link_iter) == rtnl_link_get_ifindex(bridge)) {
			...
			for (size_t i = 0; i < port_vlan_count; i++) {
				...
				snprintf(vlan_port_path, PATH_MAX, "bridge-vlan/vlan[vid='%d']/%s-ports", port_vlan_list[i].vid, port_type);

				error = lyd_new_path(*parent, ly_ctx, vlan_port_path, rtnl_link_get_name(link_iter), 0, NULL);
				if (error != LY_SUCCESS) {
					SRPLG_LOG_ERR(PLUGIN_NAME, "lyd_new_path() failed (%d): %s", error, ly_errmsg(ly_ctx));
					goto loop_cleanup;
				}
			}
		}
